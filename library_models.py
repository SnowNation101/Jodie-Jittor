...
# INITIALIZE T-BATCH VARIABLES初始化T-BATCH全局变量
def reinitialize_tbatches():
    ...
    # 这里使用了defaultdict，当访问不存在的key时，会返回一个默认值也就是空的list
    ...
    # the latest tbatch a user is in，默认值为-1
    tbatchid_user = defaultdict(lambda: -1)

    # the latest tbatch a item is in，默认值为-1
